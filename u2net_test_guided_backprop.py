...
#    loss_d2.backward()
    ...
    Sc_dx = gbp.attribute(torch.tensor(img.clone().detach()), target=(0,x,y))
    
#    Sc_dx = img.grad
    
#    gbp.backward(ids=seg)
#    Sc_dx = gbp.generate()
...
#        probs1, ids1 = gbp.forward(inputs_test)
...
#        smoothgrad_sq = GetSmoothedMask(inputs_test.clone().detach(),class_sel,magnitude=True)
        ...
        guided_backprop = returnGradGBP(inputs_test.clone().detach(),0,0).clone().detach().cpu()
        guided_backprop_i = returnGradGBP(inputs_test.clone().detach(),0,0).clone().detach().cpu()
        for x in range(len(inputs_test[0][0])):
            if (x % 25 == 0):
                print("layer:",x)
            for y in range(len(inputs_test[0][0][0])):
                if (class_sel[0][0][x][y]==1):
                    guided_backprop += returnGradGBP(inputs_test.clone().detach(),x,y).clone().detach().cpu()
                if (class_sel[0][0][x][y]==0):
                    guided_backprop_i += returnGradGBP(inputs_test.clone().detach(),x,y).clone().detach().cpu()
        guided_backprop_sq = abs(guided_backprop)
        guided_backprop_sq_i = abs(guided_backprop_i)
        ...
        imshow(torchvision.utils.make_grid(VisualizeImageGrayscale(class_sel.clone().detach().cpu())))
        plt.axis('off')
        fig.add_subplot(length, width, 3)
        guided_backprop = guided_backprop[0][0] + guided_backprop[0][1] + guided_backprop[0][2]
        imshow(torchvision.utils.make_grid(VisualizeImageGrayscale(guided_backprop)))
        plt.axis('off')
        fig.add_subplot(length, width, 4)
        guided_backprop_sq = guided_backprop_sq[0][0] + guided_backprop_sq[0][1] + guided_backprop_sq[0][2]
        imshow(torchvision.utils.make_grid(VisualizeImageGrayscale(guided_backprop_sq)))
        plt.axis('off')
        fig.add_subplot(length, width, 5)
        guided_backprop_i = guided_backprop_i[0][0] + guided_backprop_i[0][1] + guided_backprop_i[0][2]
        imshow(torchvision.utils.make_grid(VisualizeImageGrayscale(guided_backprop_i)))
        plt.axis('off')
        fig.add_subplot(length, width, 6)
        guided_backprop_sq_i = guided_backprop_sq_i[0][0] + guided_backprop_sq_i[0][1] + guided_backprop_sq_i[0][2]
        imshow(torchvision.utils.make_grid(VisualizeImageGrayscale(guided_backprop_sq_i)))
        plt.axis('off')
